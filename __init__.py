...
# TODO: alert user when multiple chain cross(same parent but not end of chain, like a branch)
...
def apply_scale(obj):
    override = {"active_object": obj, "object": obj, "selected_editable_objects": [obj]}
        ...
        for i in range(v_num):
            ...
            bpy.ops.mesh.extrude_region_move(
                TRANSFORM_OT_translate={
                    "value": (0, 0, -height_temp),
                    "orient_type": "GLOBAL",
                }
            )
            scale_factor = 1.03
            bpy.ops.transform.resize(
                value=(scale_factor, scale_factor, scale_factor), orient_type="GLOBAL"
            )
...
def create_bone_from_guide_mesh(context):
    ...
    circle_seg_num = (
        max(
            [
                int(re.search(r"b_(\d+)_(\d+)", temp).group(1))
                for temp in vg_name_list
                if temp.startswith("b_")
            ]
        )
        + 1
    )
    verticle_seg_num = max(
        [
            int(re.search(r"b_(\d+)_(\d+)", temp).group(2))
            for temp in vg_name_list
            if temp.startswith("b_")
        ]
    )
    ...
    armature_obj = bpy.data.objects.new(
        f"Armature_{basename}_{random_suffix}", armature
    )
    ...
    root_location = sum([temp.co for temp in root_vertex_list], Vector()) / len(
        root_vertex_list
    )
...
class SkirtRigidGenCreateRigidFromBoneOperator(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        for i, chain in enumerate(chain_list):
            ...
            if parent_bone:
                layout.label(
                    text=f"{i+1}.Parent:{parent_bone.name}    Chain length: {len(chain)}",
                    icon="CHECKMARK",
                )
            else:
                layout.label(
                    text=f"{i+1}.Parent:None    Chain length: {len(chain)}",
                    icon="CANCEL",
                )
            layout.label(
                text=f"    Bone list: {' || '.join(chain)}", icon="FILE_PARENT"
            )
    ...
    def execute(self, context):
        ...
        # profiled_my_function = profiler(create_rigid_from_bone)
        # profiled_my_function(context)  # 使用实际参数调用函数
...
def create_rigid_from_bone(context):
    ...
    avg_bone_length = sum(
        [(temp.head_local - temp.tail_local).length for temp in armature_obj.data.bones]
    ...
    rigid_root_obj_list = []
    nc_joint_obj_list = []
    parent_bone_name_to_rigid_objs = {}
    ...
    for i, chain in enumerate(chain_list):
        ...
        for j, bone_name in enumerate(chain):
            ...
            rotation_matrix = (
                mathutils.Matrix((bone_x_axis, bone_y_axis, bone_z_axis))
                .to_3x3()
                .transposed()
                @ mathutils.Matrix((X, Y, Z)).to_3x3()
            )
            ...
            if j == 0:
                ...
                rigid_root_obj = bpy.data.objects.new(
                    f"r_{i}_root_rigid_{random_suffix}", mesh
                )
                ...
                rigid_root_obj.matrix_world = (
                    rotation_matrix.to_4x4() @ rigid_root_obj.matrix_world
                )
            ...
            if j == 0:
                ...
                rigid_root_obj.rigid_body.collision_collections = tuple(
                    False for _ in range(19)
                ) + (True,)
            ...
            # record parent bone and rigid body, used in parent relationship
            if parent_bone_name:
                if parent_bone_name not in parent_bone_name_to_rigid_objs:
                    parent_bone_name_to_rigid_objs[parent_bone_name] = []
                parent_bone_name_to_rigid_objs[parent_bone_name].append(rigid_obj)
            ...
            if angle_limit_type == "constant":
                ...
                current_weight = j + 1
                rigid_rad_angle_out_single = (
                    rigid_rad_angle_out * current_weight / total_n
                )
                rigid_rad_angle_in_single = (
                    rigid_rad_angle_in * current_weight / total_n
                )
                rigid_circ_angle_single = rigid_circ_angle * current_weight / total_n
            else:
                raise Exception("unknow type")

            rigid_circ_angle_single = min(rigid_circ_angle_single, 180)
            rigid_rad_angle_out_single = min(rigid_rad_angle_out_single, 180)
            rigid_rad_angle_in_single = min(rigid_rad_angle_in_single, 180)
            ...
            bpy.context.object.rigid_body_constraint.spring_damping_ang_x = (
                chain_spring_damping
            )
            bpy.context.object.rigid_body_constraint.spring_damping_ang_y = (
                chain_spring_damping
            )
            bpy.context.object.rigid_body_constraint.spring_damping_ang_z = (
                chain_spring_damping
            )
            bpy.context.object.rigid_body_constraint.spring_stiffness_ang_x = (
                chain_spring_stiffness
            )
            bpy.context.object.rigid_body_constraint.spring_stiffness_ang_y = (
                chain_spring_stiffness
            )
            bpy.context.object.rigid_body_constraint.spring_stiffness_ang_z = (
                chain_spring_stiffness
            )
    ...
    if disable_self_collision:
        for rigid_obj, rigid_obj_temp in itertools.combinations(rigid_obj_list, 2):
            i = re.search(r"(\d+)_(\d+)", rigid_obj.name).group(1)
            j = re.search(r"(\d+)_(\d+)", rigid_obj.name).group(2)
            i_temp = re.search(r"(\d+)_(\d+)", rigid_obj_temp.name).group(1)
            j_temp = re.search(r"(\d+)_(\d+)", rigid_obj_temp.name).group(2)
            if int(i_temp) == int(i):
                continue

            temp_bone_name = chain_list[int(i_temp)][int(j_temp)]
            temp_pose_bone = armature_obj.pose.bones[temp_bone_name]
            temp_length = temp_pose_bone.length
            bone_name = chain_list[int(i)][int(j)]
            pose_bone = armature_obj.pose.bones[bone_name]
            length = pose_bone.length

            # skip far one
            if rigid_size_type == "relative":
                temp_width_factor = rigid_width
                temp_thickness_factor = rigid_thickness
                temp_width = temp_width_factor * temp_length
                temp_thickness = temp_thickness_factor * temp_length
                ...
                temp_width, width = rigid_width, rigid_width
                temp_thickness, thickness = rigid_thickness, rigid_thickness
            else:
                raise Exception("unexpect type")
            ...
            max_distance = (
                ...
                (width / 2) ** 2 + (thickness / 2) ** 2 + (length / 2) ** 2
            ) ** 0.5
            if (rigid_obj.location - rigid_obj_temp.location).length > max_distance:
                continue

            # create joint
            joint_obj = bpy.data.objects.new(
                f"nc_{i}_{j}&{i_temp}_{j_temp}_{random_suffix}", None
            )
            ...
            bpy.context.object.rigid_body_constraint.enabled = False
    ...
    # child of will cause a animation playback shift, parent method will not
    for parent_bone_name, rigid_objs in parent_bone_name_to_rigid_objs.items():
        bpy.ops.object.mode_set(mode="OBJECT")
        bone = armature_obj.data.bones[parent_bone_name]
        armature_obj.data.bones.active = bone

        collection_to_is_visible = {}
        if int(bpy.app.version[0]) == 4:
            for bone_collection in armature_obj.data.collections:
                collection_to_is_visible[bone_collection] = bone_collection.is_visible
        ...
        for bone_collection, is_visible in collection_to_is_visible.items():
            bone_collection.is_visible = is_visible

        # avoid the hidden active bone affect user operation
        armature_obj.data.bones.active = None
        bone.select = False


bl_info = {
    "name": "Skirt Rigid Generator",
    "author": "Oimoyu",
    "version": (1, 4),
    "blender": (3, 3, 2),
    "location": "View3D > Sidebar > Skirt Rigid Gen",
...
class SkirtRigidGenGeneratePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.scale_y = 2.0
        row.operator(
            "skirt_rigid_gen.create_guide_mesh",
            text="Generate Guide Mesh",
            icon="MOD_LATTICE",
        )
        ...
        row.scale_y = 2.0
        row.operator(
            "skirt_rigid_gen.create_bone_from_guide_mesh",
            text="Create Bone From Guide Mesh",
            icon="BONE_DATA",
        )
        row.enabled = (
            len(bpy.context.selected_objects) != 0
            and bpy.context.selected_objects[-1].get("is_guide_mesh") == True
        )
        ...
        rigid_angle_limit_box.row().prop(
            settings, "rigid_circ_angle", text="Circ Angle"
        )
        rigid_angle_limit_box.row().prop(
            settings, "rigid_rad_angle_in", text="Radial Angle In"
        )
        rigid_angle_limit_box.row().prop(
            settings, "rigid_rad_angle_out", text="Radial Angle Out"
        )
        ...
        rigid_spring_setting_box.row().prop(
            settings, "spring_setting_type", expand=True
        )
        ...
        row.scale_y = 2.0
        row.operator(
            "skirt_rigid_gen.create_rigid_from_bone",
            text="Generate Rigid Body",
            icon="MESH_CUBE",
        )
        row.enabled = (
            len(bpy.context.selected_objects) != 0
            and bpy.context.selected_objects[-1].type == "ARMATURE"
            and bpy.context.selected_objects[-1].mode == "POSE"
        )
...
class SkirtRigidGenClearAllOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        all_addon_obj_list = [
            temp for temp in bpy.data.objects if temp.get("is_skirt_rigid_gen")
        ]
        obj_with_id_list = [
            temp for temp in all_addon_obj_list if temp.get("skirt_rigid_gen_id")
        ]
...
class SkirtRigidGenToolPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.operator(
            "skirt_rigid_gen.generate_passive_bone_rigid",
            text="Generate Passive Bone Rigid",
            icon="MESH_ICOSPHERE",
        )
        row.enabled = (
            len(bpy.context.selected_objects) != 0
        ...
        layout.row().operator(
            "skirt_rigid_gen.clear_frame_cache", text="Clear Frame Cache", icon="TRASH"
        )
...
class SkirtRigidGenGeneratePassiveBoneRigidOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        selected_pose_bones = [
            bone for bone in armature_obj.pose.bones if bone.bone.select
        ]
        ...
        for pose_bone in selected_pose_bones:
            ...
            length = pose_bone.length
            bpy.ops.mesh.primitive_cube_add(
                size=length, enter_editmode=False, align="WORLD"
            )
            ...
            rigid_obj.location = armature_obj.matrix_world @ (
                (pose_bone.head + pose_bone.tail) / 2
            )
...
def get_select_rigid_body_list():
    return [
        temp
        for temp in bpy.context.selected_objects
        if temp.get("is_skirt_rigid_gen") and temp.rigid_body
    ]
...
def update_rigid_body_mass(self, context):
    ...
    for obj in select_rigid_body_list:
        obj.rigid_body.mass = (
            context.scene.skirt_rigid_gen_settings.rigid_body_mass_temp
        )
...
def update_rigid_body_friction(self, context):
    ...
    for obj in select_rigid_body_list:
        obj.rigid_body.friction = (
            context.scene.skirt_rigid_gen_settings.rigid_body_friction_temp
        )
...
def get_select_joint_list():
    return [
        temp
        for temp in bpy.context.selected_objects
        if temp.get("is_skirt_rigid_gen") and temp.rigid_body_constraint
    ]
...
def update_joint_stiffness(self, context):
    ...
    for obj in select_joint_list:
        chain_spring_stiffness = (
            context.scene.skirt_rigid_gen_settings.chain_spring_stiffness_temp
        )
...
def update_joint_damping(self, context):
    ...
    for obj in select_joint_list:
        chain_spring_damping = (
            context.scene.skirt_rigid_gen_settings.chain_spring_damping_temp
        )
...
def refresh_rigid_group(context):
    ...
    object_list = [
        temp
        for temp in bpy.context.scene.objects
        if temp.get("is_skirt_rigid_gen")
        and temp.get("skirt_rigid_gen_id")
        and temp.get("skirt_rigid_gen_basename")
    ]
...
class SkirtRigidGenHandleRigidGroupOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        obj_list = [
            temp
            for temp in bpy.data.objects
            if temp.get("skirt_rigid_gen_id") == rigid_group_item.id
        ]
        ...
        if self.action == "select":
            for obj in obj_list:
                if not self.obj_type:
                    ...
                    if self.obj_type == "all_joint" and "joint" in obj.get(
                        "skirt_rigid_gen_type", ""
                    ):
                        obj.select_set(True)
                        bpy.context.view_layer.objects.active = obj
                    else:
...
class SkirtRigidGenModifyPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        selected_object_list = [
            temp
            for temp in selected_object_list
            if temp.get("is_skirt_rigid_gen") and temp.get("skirt_rigid_gen_id")
        ]
        ...
        joint_list = [
            temp
            for temp in selected_object_list
            if temp.type == "EMPTY" and temp.rigid_body_constraint
        ]

        selected_object_group = group_by_attr(
            selected_object_list, "skirt_rigid_gen_id"
        )
        ...
        col.operator(
            "skirt_rigid_gen.refresh_rigid_group", text="", icon="FILE_REFRESH"
        )
        op = col.operator(
            "skirt_rigid_gen.handle_rigid_group", text="", icon="HIDE_OFF"
        )
        op.action = "show"
        op = col.operator("skirt_rigid_gen.handle_rigid_group", text="", icon="HIDE_ON")
        op.action = "hide"
        op = col.operator(
            "skirt_rigid_gen.handle_rigid_group", text="", icon="RESTRICT_SELECT_OFF"
        )
        op.action = "select"
...
class SkirtRigidGenPanelSettings(bpy.types.PropertyGroup):
    ...
    rigid_thickness: bpy.props.FloatProperty(
        name="rigid thickness", min=0.001, default=1
    )
    ...
    rigid_damping: bpy.props.FloatProperty(
        name="Rigid Damping", default=0.5, min=0, max=1
    )

    rigid_rad_angle_out: bpy.props.FloatProperty(
        name="Radial Angle Out",
        min=0,
        default=180,
        description="Angle limit outward along the radial direction",
    )
    rigid_rad_angle_in: bpy.props.FloatProperty(
        name="Radial Angle In",
        min=0,
        default=180,
        description="Angle limit inward along the radial direction",
    )
    rigid_circ_angle: bpy.props.FloatProperty(
        name="Circ Angle",
        min=0,
        default=180,
        description="Angular limits along the circumferential direction",
    )

    angle_limit_type: bpy.props.EnumProperty(
        name="Angle Limit Type",
        items=(
            ("linear", "Linear", ""),
            ("constant", "Constant", ""),
        ),
    ...
    spring_setting_type: bpy.props.EnumProperty(
        name="Spring Setting Type",
        items=(
            ("linear", "Linear", ""),
            ("constant", "Constant", ""),
        ),
    ...
    guide_mesh_type: bpy.props.EnumProperty(
        name="Guide Mesh Type",
        items=(
            ("tube", "Tube", ""),
            ("face", "Face", ""),
            ("line", "Line", ""),
    ...
    rigid_size_type: bpy.props.EnumProperty(
        name="Rigid Size Type",
        items=(
            ("relative", "Relative", ""),
            ("absolute", "Absolute", ""),
        ),
    ...
    chain_spring_stiffness: bpy.props.FloatProperty(
        name="chain sping stiffness",
        min=0,
        default=10,
        description="Horizontal Spring Stiffness",
    )
    chain_spring_damping: bpy.props.FloatProperty(
        name="chain sping damping",
        min=0,
        default=0.5,
        description="Horizontal Spring Damping",
    )

    disable_self_collision: bpy.props.BoolProperty(
        name="Disable Self Collistion",
        default=False,
        description="This option can only be enabled when the number of rigid bodies is less than 32",
    )

    rigid_body_mass_temp: bpy.props.FloatProperty(
        name="Temp Rigid Body Mass", default=1.0, min=0.0, update=update_rigid_body_mass
    )
    rigid_body_friction_temp: bpy.props.FloatProperty(
        name="Temp Rigid Body Friction",
        default=0.5,
        min=0.0,
        max=1,
        update=update_rigid_body_friction,
    )

    chain_spring_stiffness_temp: bpy.props.FloatProperty(
        name="Temp Chain Spring Stiffness",
        default=1.0,
        min=0.0,
        update=update_joint_stiffness,
    )
    chain_spring_damping_temp: bpy.props.FloatProperty(
        name="Temp Chain Spring Damping",
        default=0.5,
        min=0.0,
        update=update_joint_damping,
    )
...
def register():
    ...
    bpy.types.Scene.skirt_rigid_gen_settings = bpy.props.PointerProperty(
        type=SkirtRigidGenPanelSettings
    )
